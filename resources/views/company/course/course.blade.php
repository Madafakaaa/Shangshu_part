...
<div class="container-fluid mt-4">
  <div class="row mb-3">
    <div class="col-auto">
      ...
      <button class="btn btn-sm btn-outline-danger btn-round btn-icon" data-toggle="tooltip" data-original-title="批量结课" onclick="batchConfirm('/company/course/delete',  'delete-button', '确认批量删除所选课程？')">
          ...
          <table class="table table-hover datatable-basic text-center table-bordered">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td><strong>{{ number_format($course->course_unit_price, 1) }}元</strong></td>
                <td>
                ...
                <td>
                  @if($course->course_is_available==1)
                    ...
                    <button type="button" class="btn btn-outline-danger btn-sm delete-button" id='delete_button_{{$loop->iteration}}' onclick="buttonConfirm('delete_button_{{$loop->iteration}}', '/company/course/delete?id={{encode($course->course_id, 'course_id')}}', '确认结束课程？')">结课</button>
                  @else
