              ...
              <tr>
                ...
                <td>
                  @if($daycare_refund['daycare_refund_reviewed_status']==0)
                    <button type="button" class="btn btn-warning btn-sm" id='review_button_{{$loop->iteration}}' onclick="buttonConfirm('review_button_{{$loop->iteration}}', '/finance/refundPayment/review?id={{encode($daycare_refund['daycare_refund_id'], 'daycare_refund_id')}}', '确认复核所选退费记录？')">复核</button>
                ...
                <td>
                  @if($daycare_refund['daycare_refund_reviewed_status']==0)
                    <button type="button" class="btn btn-outline-danger btn-sm" id='delete_button_{{$loop->iteration}}' onclick="buttonConfirm('delete_button_{{$loop->iteration}}', '/finance/refundPayment/delete?id={{encode($daycare_refund['daycare_refund_id'], 'daycare_refund_id')}}', '确认删除所选退费记录？')">删除</button>
                  @endif
                </td>
              </tr>
              @endforeach
