...
@section('nav')
<h2 class="text-white d-inline-block mb-0">家长会记录</h2>
<nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
  <ol class="breadcrumb breadcrumb-links breadcrumb-dark">
    <li class="breadcrumb-item"><a href="/home"><i class="fas fa-home"></i></a></li>
    <li class="breadcrumb-item active">教务中心</li>
    <li class="breadcrumb-item active">家长会记录</li>
  </ol>
</nav>
@endsection
...
<div class="container-fluid mt-3">
  <div class="row mb-3">
    <div class="col-auto">
      <a href="/education/meeting/create" class="btn btn-sm btn-neutral btn-round btn-icon" data-toggle="tooltip" data-original-title="添加记录">
      ...
      <div class="card mb-4">
        ...
        <div class="table-responsive py-3">
          <table class="table table-hover datatable-basic text-left table-bordered">
            ...
            <tbody>
              @foreach ($db_meetings as $meeting)
              <tr>
                <td>{{ $loop->iteration }}</td>
                <td>{{ $meeting->department_name }}</td>
                <td>
                  @if($meeting->student_gender=="男")
                    <img src="{{ asset(_ASSETS_.'/img/icons/male.png') }}" style="height:20px;">
                  @else
                    <img src="{{ asset(_ASSETS_.'/img/icons/female.png') }}" style="height:20px;">
                  @endif
                  <a href="/student?id={{encode($meeting->student_id, 'student_id')}}">{{ $meeting->student_name }}</a>
                </td>
                <td>
                ...
                <td>
                  ...
                  <button type="button" class="btn btn-outline-danger btn-sm delete-button" id='delete_button_{{$loop->iteration}}' onclick="buttonConfirm('delete_button_{{$loop->iteration}}', '/education/meeting/delete?id={{encode($meeting->meeting_id, 'meeting_id')}}', '是否确认删除所选记录？')">删除</button>
                </td>
              </tr>
              @endforeach
            </tbody>
