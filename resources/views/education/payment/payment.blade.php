...
<div class="container-fluid mt-3">
  <div class="row mb-3">
    <div class="col-auto">
      <button type="button" class="btn btn-sm btn-outline-warning btn-round btn-icon" id='review_all_button' onclick="buttonConfirm('review_all_button', '/education/payment/review/all', '确认复核所有可复核购课记录？')">
        <span class="btn-inner--icon"><i class="ni ni-check-bold"></i></span>
        <span class="btn-inner--text">全部复核</span>
      </button>
      <button class="btn btn-sm btn-outline-danger btn-round btn-icon" data-toggle="tooltip" data-original-title="批量删除" onclick="batchConfirm('/education/payment/delete', 'delete-button', '删除后将扣除学生课时，若学生剩余课时不足将无法扣除。是否确认删除购课记录')">
              ...
              <tr>
                ...
                <td>
                  @if($payment['review_user_name']=="")
                    ...
                    @else
                      <button type="button" class="btn btn-warning btn-sm" id='review_button_{{$loop->iteration}}' onclick="buttonConfirm('review_button_{{$loop->iteration}}', '/education/payment/review?id={{encode($payment['payment_id'], 'payment_id')}}', '确认复核所选购课记录？')">复核</button>
                    @endif
                  @else
                ...
                <td>
                  @if($payment['review_user_name']=="")
                    <button type="button" class="btn btn-outline-danger btn-sm" id='delete_button_{{$loop->iteration}}' onclick="buttonConfirm('delete_button_{{$loop->iteration}}', '/education/payment/delete?id={{encode($payment['payment_id'], 'payment_id')}}', '确认删除所选购课记录？')">删除</button>
                  @endif
                </td>
              </tr>
              @endforeach
