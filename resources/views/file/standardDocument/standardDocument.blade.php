          ...
          <table class="table table-hover datatable-basic text-left table-bordered">
            ...
            <tbody>
              ...
              <tr>
                ...
                @if($row->standard_document_semester==1)
                  <td>秋季班</td>
                @elseif($row->standard_document_semester==2)
                  <td>春季班</td>
                @elseif($row->standard_document_semester==3)
                  <td>寒假班</td>
                @else
                  <td>暑假班</td>
                @endif
                <td>{{ date('Y-m-d', strtotime($row->standard_document_create_time)) }}</td>
                <td>
                  <a href="/file/standardDocument/document?id={{encode($row->standard_document_id, 'standard_document_id')}}"><button type="button" class="btn btn-primary btn-sm">下载教案</button></a>
                  <a href="/file/standardDocument/answer?id={{encode($row->standard_document_id, 'standard_document_id')}}"><button type="button" class="btn btn-outline-primary btn-sm">下载答案</button></a>
                </td>
              </tr>
              @endforeach
            </tbody>
