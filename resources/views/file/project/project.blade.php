...
<nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
  <ol class="breadcrumb breadcrumb-links breadcrumb-dark">
    ...
    <li class="breadcrumb-item active">教学计划</li>
  </ol>
</nav>
@endsection
...
<div class="container-fluid mt-3">
  <div class="row mb-3">
    <div class="col-auto">
      <a href="/file/project/create" class="btn btn-sm btn-neutral btn-round btn-icon" data-toggle="tooltip" data-original-title="添加教案">
          ...
          <table class="table table-hover datatable-basic text-left table-bordered">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td><a href="/file/project/download?id={{encode($row->project_id, 'project_id')}}">{{ $row->project_name }}</a></td>
                <td>{{ $row->subject_name }}</td>
                <td>{{ $row->grade_name }}</td>
                <td>{{ $row->project_semester }}</td>
                <td>
                  <a href="/file/project/download?id={{encode($row->project_id, 'project_id')}}"><button type="button" class="btn btn-primary btn-sm">教案下载</button></a>
                  <a href="/file/project/answer/download?id={{encode($row->project_id, 'project_id')}}"><button type="button" class="btn btn-primary btn-sm">答案下载</button></a>
                  <a href="/file/project/edit?id={{encode($row->project_id, 'project_id')}}"><button type="button" class="btn btn-primary btn-sm">修改</button></a>
                  <button type="button" class="btn btn-outline-danger btn-sm delete-button" id='delete_button_{{$loop->iteration}}' onclick="buttonConfirm('delete_button_{{$loop->iteration}}', '/file/project/delete?id={{encode($row->project_id, 'project_id')}}', '是否确认删除所选教案？')">删除</button>
                </td>
              </tr>
              @endforeach
            </tbody>
